...
const createProduct = async function (req, res) {
    try {
        ...
        if (!isValidPrice(price.trim()))
            return res.status(400).send({ status: false, message: "Enter a proper price" })
        ...
        if (Object.keys(data).some(a => a === "availableSizes")) {  // .some method iterate on the array and find the value and return the boolean value
            if (!availableSizes)
                return res.status(400).send({ status: false, message: "available sizes can't be empty" })
        ...
        if (installments) {
            ...
            if (!(/^[0-9]+$/.test(installments)))
                return res.status(400).send({ status: false, message: "Invalid value for installments" })
        ...
        if (style) {
            ...
            if (!isValidStyle(style.trim()))
                return res.status(400).send({ status: false, message: "Enter a proper style" })
            product.style = style
        }
...
const productUpdate = async function (req, res) {
    try {
        ...
        if (!isValidRequestBody(updateData) && (files == undefined))
            return res.status(400).send({ status: false, message: "update Body can't be empty" });
        ...
        if (Object.keys(updateData).some(a => a === "title")) {
            if (!title)
                return res.status(400).send({ status: false, message: "Enter Value in title Field " })
            if (!isValid(title))
                return res.status(400).send({ status: false, message: "Please enter title in correct format" })
            ...
            if (findTitle)
                return res.status(400).send({ status: false, message: "This Title Is Alredy Used" })
        }

        //validation of description
        if (Object.keys(updateData).some(a => a === "description")) {
            if (!description)
                return res.status(400).send({ status: false, message: "Enter Value in description Field " })
        ...
        if (Object.keys(updateData).some(a => a === "price")) {
            if (!price)
                return res.status(400).send({ status: false, message: "Enter Value in price Field " })
        ...
        if (Object.keys(updateData).some(a => a === "currencyId")) {
            if (!currencyId)
                return res.status(400).send({ status: false, message: "Enter Value in currencyId Field " })
            currencyId = currencyId.trim().toUpperCase()
        ...
        if (Object.keys(updateData).some(a => a === "currencyFormat")) {
            if (!currencyFormat)
                return res.status(400).send({ status: false, message: "Enter Value in currencyFormat Field " })
        ...
        if (Object.keys(updateData).some(a => a === "isFreeShipping")) {
            if (!isFreeShipping)
                return res.status(400).send({ status: false, message: "Enter Value in isFreeShipping Field " })
        ...
        if (productImage == '') {
            if (!productImage && files.length == 0) {
                return res.status(400).send({ status: false, message: "Enter Value in productImage file " })
            }
        }
        ...
        if (files.length > 0) {
            let fileExtension = files[0]
            if (!isValidImage(fileExtension.originalname))
                return res.status(400).send({ status: false, message: "productImage format Must be in jpeg,jpg,png" })
        ...
        if (Object.keys(updateData).some(a => a === "style")) {
            if (!style)
                return res.status(400).send({ status: false, message: "Enter Value in style Field " })
        ...
        if (Object.keys(updateData).some(a => a === "installments")) {
            if (!installments)
                return res.status(400).send({ status: false, message: "Enter Value in installments Field " })
