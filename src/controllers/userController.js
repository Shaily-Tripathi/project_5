...
const createUser = async function (req, res) {
    try {
        ...
        if (!isValidName(fname.trim()))
            return res.status(400).send({ status: false, message: "fname is not Valid or Empty" });

        // lname validation
        if (!lname)
            return res.status(400).send({ status: false, message: "lname is required" });
        if (!isValidName(lname.trim()))
            return res.status(400).send({ status: false, message: "lname is not Valid or Empty" });
        ...
        if (!isValidImage(fileExtension.originalname))
            return res.status(400).send({ status: false, message: "Image format Must be in jpeg,jpg,png" })
        ...
        if (!isValidPhone(phone)) return res.status(400).send({ status: false, message: "phone is not Valid or Empty" });

        const duplicatePhone = await userModel.findOne({ phone: phone });
        if (duplicatePhone) return res.status(400).send({ status: false, message: "This phone number is Already Used !" })
        ...
        return res.status(201).send({ status: true, message: "User is created Sucessfully", data: user })
...
const loginUser = async function (req, res) {
    try {
        ...
        if (!isValidRequestBody(requestBody))
            return res.status(400).send({ status: false, message: "requestBody can't be empty" })
        ...
        if (!user)
            return res.status(400).send({ status: false, message: "Email id not valid" })
...
const getUserDetails = async function (req, res) {
    try {
        ...
        if (!userId)
            return res.status.send({ status: false, message: "Please Provide userId in the path params" })
...
const updateUser = async function (req, res) {
    try {
        ...
        if (Object.keys(data).some(a => a === "fname")) {
            if (!fname)
                return res.status(400).send({ status: false, message: "Enter Value in Fname Field " })
        ...
        if (Object.keys(data).some(a => a === "lname")) {
            if (!lname)
                return res.status(400).send({ status: false, message: "Enter Value in lname Field " })

            if (!isValidName(lname))
                return res.status(400).send({ status: false, message: "please write lname in correct way" })
        }
        // =================Email validation==========================
        if (Object.keys(data).some(a => a === "email")) {
            if (!email)
                return res.status(400).send({ status: false, message: "Enter Value in email Field " })

            if (!isValidEmail(email))
                return res.status(400).send({ status: false, message: "please write email in correct way" })

            const checkEmail = await userModel.findOne({ email: email });
            if (checkEmail)
                return res.status(400).send({ status: false, message: "This email is alredy exist" })
        }
        ...
        if (Object.keys(data).some(a => a === "password")) {
            ...
            if (!isValidPass(password))
                return res.status(400).send({ status: false, message: "please write password in correct way" })
        ...
        if (Object.keys(data).some(a => a === "phone")) {
            if (!phone)
                return res.status(400).send({ status: false, message: "Enter Value in phone Field " })

            if (!isValidPhone(phone))
                return res.status(400).send({ status: false, message: "please write phone in correct way" })
        ...
        if (profileImage == '' && files.length == 0) {
            return res.status(400).send({ status: false, message: "Enter Value in profileImage file " })
        }
